...
\begin{definition}
\index{Lattice}
    A \emph{lattice} is a partially ordered set $\mathcal{L}$ in which 
    each pair of elements $x,y\in \mathcal{L}$ has a least upper bound $x\vee y$ and a
...
\index{Lattice!of subgroups}
    Let $G$ be a group and $\mathcal{L}(G)$ be the collection of subgroups of $G$. The relation
    \[ 
    H\leq K\Longleftrightarrow H\subseteq K
    \] 
    turns $\mathcal{L}(G)$ into a lattice 
    with $H\vee K=\langle H,K\rangle$ and 
    $H\wedge K=H\cap K$. 
\end{example}

\begin{example}
    Let $E/K$ be a field extension and $\mathcal{L}(E/K)$ be the collection of 
    intermediate fields. The relation
    \[
    F\leq L\Longleftrightarrow F\subseteq L
    \]
    turns $\mathcal{L}(E/K)$ into a lattice with 
...
A map $f\colon \mathcal{L}_1\to \mathcal{L}_2$ between two lattices is 
...
\label{xca:order_reversing}
    Let $\mathcal{L}_1$ and $\mathcal{L}_2$ be lattices and $f\colon \mathcal{L}_1\to \mathcal{L}_2$ be a bijection 
...
\index{Galois' theorem}
    ...
    \[
    \mathcal{L}(E/K)=\{F:K\subseteq F\subseteq E\text{ subfields}\}\leftrightarrow
    \{S:S\text{ is a subgroup of $G$}\}=\mathcal{L}(G). 
    \]
...
Let 
\begin{align*}
&\alpha\colon \mathcal{L}(E/K)\to \mathcal{L}(G),\quad\alpha(F)=\Gal(E/F),\\
&\beta\colon \mathcal{L}(G)\to \mathcal{L}(E/K),\quad\beta(S)=\prescript{S}{}{E}.
\end{align*}
A routine
