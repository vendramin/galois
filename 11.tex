...
\begin{proof}
    ...
    \[
    \sigma\tau(\alpha)=\sigma(\tau(\alpha))=\sigma(\omega_{\sigma}\alpha)=\omega_\tau\sigma(\alpha)=\omega_{\tau}\omega_{\sigma}\alpha.
    \]
...
\begin{proof}
    ...
    $\trace{E/K}(1)=p1=0$, Hilbert's theorem 
...
\]
where $s_i=\prod_{A_i}\prod_{j\in A_i}t_j$ and 
$A_i$ runs over all subsets of $\{1,\dots,n\}$ of size $i$. 
